...
export function AppProvider({ children }) {
  ...
  const buildKOTHtml = useCallback((kot, tableNo, items, printerLabel) => {
    ...
    const pageHeight = Math.max(110, 75 + (itemCount * 14));
    ...
    <html>
      <head>
        ...
        <style>
          ...
          .header { text-align: center; font-weight: 800; margin-bottom: 6px; font-size: ${subFontSize}px; }
          .sub { text-align: center; font-size: ${subFontSize}px; margin-bottom: 4px; font-weight: 700; }
          .divider { border-top: 1.5px dashed #000; margin: 6px 0; }
          .item { display: flex; justify-content: space-between; margin: 8px 0; font-weight: 700; font-size: ${fontSize}px; line-height: 1.4; }
          .qty { font-weight: 800; min-width: ${qtyWidth}px; text-align: ${qtyAlign}; font-size: ${fontSize + 1}px; }
          .note { font-size: ${noteFontSize}px; margin: 2px 0 6px 8px; border-left: 2px solid #000; padding-left: 5px; font-weight: 600; }
        </style>
      </head>
      <body>
        <div class="header" style="font-size: ${headerFontSize}px;">${printerLabel.toUpperCase()} KOT</div>
  ...
  const printBillDocument = useCallback(async (
    ...
    const isServiceTaxOn = table?.serviceTaxEnabled !== undefined
      ...
      : (typeof table?.serviceTax === 'number' && table.serviceTax > 0 ? true : !!settings.serviceTaxEnabled);
    ...
    if (hasAlcohol) {
      if (typeof table.discountPercent === 'number' && table.discountPercent > 0) {
        ...
        discountAmount = typeof table.discountAmount === 'number' ? table.discountAmount : Number((alcoholSubtotal * (discountPercent / 100)).toFixed(2));
      } else {
        ...
        if (dv.endsWith('%')) {
          ...
          discountAmount = Number((alcoholSubtotal * (discountPercent / 100)).toFixed(2));
          ...
          discountPercent = alcoholSubtotal > 0 ? Math.min(100, Math.round((discountAmount / alcoholSubtotal) * 100)) : 0;
        }
      }
      ...
      } else if (foodSubtotal === 0 && ((((table?.grandTotal !== undefined && table.grandTotal <= 1) || (total !== undefined && total <= 1)) && discountAmount > 0) || discountPercent >= 100)) {
        discountPercent = 100;
      }
    } else {
      discountAmount = 0;
    ...
    const grandTotal = discountPercent >= 100 && foodSubtotal === 0 && Math.round(totalBeforeDiscount) > 1 ? (1 + fine) : Math.max(0, Math.round(rawTotal));
  ...
  useEffect(() => {
    ...
    newSocket.on('INVENTORY_UPDATED', (data) => {
      ...
      safeFetch(apiUrl('/api/menu')).then(res => res.json()).then(mList => {
        if (Array.isArray(mList)) setMenu(mList);
      }).catch(() => {});
    });

    newSocket.on('MENU_UPDATED', (menuData) => {
  ...
  const billTotals = useMemo(() => {
    ...
    let discountAmount = 0;
    if (hasAlcohol && dv) {
      const discountVal = parseFloat(dv.replace(/[^0-9.]/g, '')) || 0;
      if (discountVal >= 100) {
        if (foodSubtotal === 0) {
          const rounded = Math.round(totalBeforeDiscount);
          discountAmount = rounded > 1 ? rounded - 1 : 0;
        } else {
          discountAmount = Number(alcoholSubtotal.toFixed(2));
        }
  ...
  const getTableInfo = useCallback((tableId) => {
    ...
    const hasAlcohol = alcoholSubtotal > 0;
    const discountVal = hasAlcohol ? (parseFloat(String(localBill?.discount || (actOrder?.discount ? actOrder.discount : '') || '').replace(/[^0-9.]/g, '')) || 0) : 0;
    let discountAmount = 0;
  ...
  }, [tableBills, activeSessions, settings, allSellableItems, inventory]);
  ...
  const openTableSession = useCallback(async (tableNo, waiterName = '', orderType = 'dine-in', customerName = '', customerPhone = '') => {
    try {
      const res = await authFetch(apiUrl(`/api/orders/table/${tableNo}/open`), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ waiterName, orderType, customerName, customerPhone })
      });
      if (!res.ok) {
        const errData = await res.json().catch(() => ({}));
